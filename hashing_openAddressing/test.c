...
int main( int argc, char **argv ) {
 ...
 if(my_entry) {
  printf(" - testing(): My Entry: %d = %d expecting 0 = 22\n", my_entry->key, my_entry->value);
 }

 my_entry = oa_search(my_table, 12);
 if(my_entry) {
  printf(" - testing(): My Entry: %d = %d expecting 12 = 32\n", my_entry->key, my_entry->value);
 }

 my_entry = oa_search(my_table, 5);
 if(my_entry) {
  printf(" - testing(): My Entry: %d = %d expecting 5 = 36\n", my_entry->key, my_entry->value);
 }

 my_entry = oa_search(my_table, 89);
 if(my_entry) {
  printf(" - testing(): My Entry: %d = %d expecting  89 = 27\n", my_entry->key, my_entry->value);
 }

 my_entry = oa_search(my_table, 52);
 if(my_entry) {
  printf(" - testing(): My Entry: %d = %d expecting 52 = 53\n", my_entry->key, my_entry->value);
 }

 my_entry = oa_search(my_table, 51);
 if(my_entry) {
  printf(" - testing(): My Entry: %d = %d expecting 51 = 504\n", my_entry->key, my_entry->value);
 }

 if(oa_delete(my_table, 51)) {
   printf(" - testing(): Successfully deleted key of 51\n");
   my_entry = oa_search(my_table, 51);
   if(my_entry == NULL) {
     printf(" - testing(): key 51 is empty as expected\n");
   }
   my_table = oa_insert(my_table, 51, 504);
   my_entry = oa_search(my_table, 51);
   if(my_entry) {
